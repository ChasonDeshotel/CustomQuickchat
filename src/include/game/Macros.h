...
#define DELAY(delaySeconds, code) gameWrapper->SetTimeout([this](GameWrapper* gw) { code }, delaySeconds)

#define DELAY_CAPTURE(delaySeconds, code, ...) gameWrapper->SetTimeout([this, __VA_ARGS__](GameWrapper* gw) { code }, delaySeconds)

#define GAME_THREAD_EXECUTE(code)                               \
    do {                                                        \
        gameWrapper->Execute([this](GameWrapper* gw) { code }); \
    } while (0)

#define GAME_THREAD_EXECUTE_CAPTURE(code, ...)                               \
    do {                                                                     \
        gameWrapper->Execute([this, __VA_ARGS__](GameWrapper* gw) { code }); \
    } while (0)
