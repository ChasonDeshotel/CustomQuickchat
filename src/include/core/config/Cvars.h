...
constexpr CvarData autoDetectInterpreterPath =
    CVAR("auto_detect_interpreter_path", "Automatically detect python interpreter filepath");
constexpr CvarData overrideDefaultQuickchats =
    CVAR("override_default_quickchats", "override default quickchat with custom one if they share the same binding");
constexpr CvarData blockDefaultQuickchats = CVAR("block_default_quickchats", "block default quickchats (without unbinding them)");
constexpr CvarData searchForPyInterpreter =
    CVAR("search_for_python_interpreter", "search for full path to pythonw.exe, instead of using the 'pythonw' command line argument");
constexpr CvarData disablePostMatchQuickchats =
    CVAR("disable_post_match_quickchats", "disable custom quickchats in the post match screen");
...
constexpr CvarData minBindingDelay =
    CVAR("min_binding_delay", "minimum delay between binding activation (prevents accidental activation)");
...
constexpr CvarData list_custom_chat_labels =
    CVAR("list_custom_chat_labels", "list all custom chat labels that will show up in quickchat UI");
