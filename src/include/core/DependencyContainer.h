...
class DependencyContainer {
  public:
    ...
    enum class Lifetime { Transient, Scoped, Singleton };
    ...
    void registerType(Lifetime lifetime = Lifetime::Transient) {
        factories_[typeid(Interface)] = [this, lifetime](const std::type_info& type) -> std::shared_ptr<void> {
            ...
            auto createInstance = [this]() { return std::make_shared<Impl>(resolveArg<Args>()...); };
    ...
    void registerFactory(std::function<std::shared_ptr<Interface>(DependencyContainer&)> factory,
        ...
        factories_[typeid(Interface)] = [this, factory, lifetime](const std::type_info& type) -> std::shared_ptr<void> {
            ...
            auto createInstance = [this, factory]() { return factory(*this); };
    ...
    auto createScope() -> Scope { return {}; }

    void beginScope(Scope& scope) { currentScope_ = &scope; }

    void endScope() { currentScope_ = nullptr; }
  ...
  private:
    ...
    std::unordered_map<std::type_index, std::function<std::shared_ptr<void>(const std::type_info&)>> factories_;
