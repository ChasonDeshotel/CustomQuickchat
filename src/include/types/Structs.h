...
struct ButtonPress {
    ...
    // default (empty)
    ButtonPress()
      : buttonName(std::string())
      , pressedTime(std::chrono::steady_clock::time_point()) {}
...
struct Binding {
    ...
    bool ShouldBeTriggered(const ButtonPress& buttonEvent,
        ...
        switch (bindingType) {
            case EBindingType::Combination: return CheckCombination(buttonEvent, keyStates, lastChatSent, minDelayBetweenBindings);
            case EBindingType::Sequence:
                return CheckSequence(buttonEvent, lastChatSent, epochTime, minDelayBetweenBindings, maxTimeWindow);
            default: return false; // if there's no valid binding type for some reason
        }
    }
    ...
    bool CheckSequence(const ButtonPress& buttonEvent,
        ...
        if (correct1stButtonPressed) {
            ...
            firstButtonState.Reset(
                epochTime); // binding was triggered too early, just reset it (bc it prolly wasn't meant to be triggered)
        }

        return false;
    }

    // determine if chat contains any special keyword or text effect (once, at the time of binding creation, rather than every time
    ...
    static ETextEffect GetTextEffect(EKeyword keyword) {
        switch (keyword) {
            ...
            case EKeyword::SpeechToTextUwu: return ETextEffect::Uwu;
            case EKeyword::LastChatSarcasm:
            case EKeyword::SpeechToTextSarcasm: return ETextEffect::Sarcasm;
            default: return ETextEffect::None;
        }
    }
...
struct ChatterRanks {
    ...
    inline RankData get_rank(ERankPlaylists playlist) {
        switch (playlist) {
            case ERankPlaylists::Ones: return ones;
            case ERankPlaylists::Twos: return twos;
            case ERankPlaylists::Threes: return threes;
            case ERankPlaylists::Casual: return casual;
            default: return RankData();
        }
    }
    ...
    // ... but it's not what is used internally to determine matchmaking. Apparently that would be the Microsoft TrueSkill formula: Mu
    // - (3 * Sigma)
    static inline float calculate_skill_rating(float mu) { return (mu * 20) + 100; }
