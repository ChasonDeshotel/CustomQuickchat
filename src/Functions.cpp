...
void
CustomQuickchat::NotifyAndLog(const std::string& title, const std::string& message, int duration) {
    GAME_THREAD_EXECUTE_CAPTURE(Instances.SpawnNotification(title, message, duration, true);, title, message, duration);
}
...
void
CustomQuickchat::InitStuffOnLoad() {
...
void
CustomQuickchat::determine_quickchat_labels(UGFxData_Controls_TA* controls, bool log) {
    if (!controls) {
...
void
CustomQuickchat::apply_all_custom_qc_labels_to_ui(UGFxData_Chat_TA* caller) {
    if (!caller)
        return;
...
void
CustomQuickchat::apply_custom_qc_labels_to_ui(UGFxData_Chat_TA* caller, UGFxData_Chat_TA_execOnPressChatPreset_Params* params) {
    if (!caller || !params)
        return;
