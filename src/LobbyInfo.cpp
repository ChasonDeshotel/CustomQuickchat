...
void
LobbyInfoComponent::Initialize(std::shared_ptr<GameWrapper> gw) {
    gameWrapper = gw;

    // hook events
    gameWrapper->HookEventPost(
        Events::LoadingScreenStart, std::bind(&LobbyInfoComponent::event_we_need_to_yeet_stored_data, this, std::placeholders::_1));
    LOG("Hooked event: \"{}\"", Events::LoadingScreenStart);

    gameWrapper->HookEventPost(
        Events::GFxHUD_TA_Destroyed, std::bind(&LobbyInfoComponent::event_we_need_to_yeet_stored_data, this, std::placeholders::_1));
    ...
    gameWrapper->HookEventWithCallerPost<ActorWrapper>(Events::GFxData_Chat_TA_OnShellSet,
        std::bind(&LobbyInfoComponent::event_GFxData_Chat_TA_OnShellSet,
            this,
            std::placeholders::_1,
            std::placeholders::_2,
            std::placeholders::_3));
    ...
    gameWrapper->HookEventWithCallerPost<ActorWrapper>(Events::GFxData_Chat_TA_AddChatMessage,
        std::bind(&LobbyInfoComponent::event_GFxData_Chat_TA_AddChatMessage,
            this,
            std::placeholders::_1,
            std::placeholders::_2,
            std::placeholders::_3));
...
void
LobbyInfoComponent::event_GFxData_Chat_TA_OnShellSet(ActorWrapper caller, void* params, std::string event) {
...
void
LobbyInfoComponent::event_we_need_to_yeet_stored_data(std::string event) {
    clear_stored_chats();
    clear_stored_ranks();
}

void
LobbyInfoComponent::event_GFxData_Chat_TA_AddChatMessage(ActorWrapper caller, void* params, std::string event) {
...
void
LobbyInfoComponent::clear_stored_chats() {
    match_chats.clear();
    LOG("Cleared stored match chats");
}

void
LobbyInfoComponent::clear_stored_ranks() {
    match_ranks.clear();
    LOG("Cleared stored player ranks");
}

void
LobbyInfoComponent::log_chat_data(const ChatData& chat) {
    LOG("----------------------------------------------------");
...
LobbyInfoComponent::get_last_chat_data() -> ChatData {
    ...
    */
    ...
    const LastChatPreferences chat_preferences = { quickchats_in_last_chat_cvar.getBoolValue(),
...
auto
LobbyInfoComponent::get_last_chatter_ranks() -> ChatterRanks {
...
auto
LobbyInfoComponent::uid_str_from_net_id(const FUniqueNetId& id) -> std::string {
...
auto
LobbyInfoComponent::get_platform_str(uint8_t platform) -> std::string {
    switch (static_cast<EOnlinePlatform>(platform)) {
        case EOnlinePlatform::OnlinePlatform_Steam: return "Steam";
        case EOnlinePlatform::OnlinePlatform_PS4:
        case EOnlinePlatform::OnlinePlatform_PS3: return "PlayStation";
        case EOnlinePlatform::OnlinePlatform_Dingo: return "Xbox";
        case EOnlinePlatform::OnlinePlatform_OldNNX:
        case EOnlinePlatform::OnlinePlatform_NNX: return "Nintendo";
        case EOnlinePlatform::OnlinePlatform_PsyNet:
        case EOnlinePlatform::OnlinePlatform_Epic: return "Epic";
...
auto
ChatData::is_valid_last_chat(const LastChatPreferences& prefs, uint8_t user_team) const -> bool {
    ...
    if (IsUser) {
        ...
        // NOTE:
        // If a party chat was sent (aka ChatChannel == 2), Team will show up as 255 (uint8_t version of -1) even if the player's
        // actual team number is 0 or 1
