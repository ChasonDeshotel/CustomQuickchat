#include "Utils.h"
#include "../../lib/SDK/GameDefines.hpp"
#include <fstream>
#include <iostream>
#include <windows.h>
...
namespace Format {
auto
ToHexString(uintptr_t address) -> std::string {
...
auto
ToHexString(int32_t decimal_val, int32_t min_hex_digits) -> std::string {
    return std::format("0x{:0{}X}", decimal_val, min_hex_digits);
}

auto
ToASCIIString(std::string str) -> std::string {
...
auto
SplitStrByNewline(const std::string& input) -> std::vector<std::string> {
...
auto
SplitStr(const std::string& str, const std::string& delimiter) -> std::vector<std::string> {
...
auto
EscapeBraces(const std::string& str) -> std::string {
    std::string escaped;
    for (char ch : str) {
        if (ch == '{' || ch == '}') {
...
auto
GenRandomString(int length) -> std::string {
...
auto
IsStringHexadecimal(std::string str) -> bool {
    if (str.empty()) {
        return false;
    }
...
void
ToLowerInline(std::string& str) {
    std::transform(str.begin(), str.end(), str.begin(), tolower);
}

auto
RemoveAllChars(std::string str, char character) -> std::string {
    str.erase(std::remove(str.begin(), str.end(), character), str.end());
    return str;
}

void
RemoveAllCharsInline(std::string& str, char character) {
    str.erase(std::remove(str.begin(), str.end(), character), str.end());
}

auto
TrimBeginningAndEndWhitespace(const std::string& str) -> std::string {
    if (str.empty())
        return str;
...
auto
ToHex(void* address, bool bNotation) -> std::string {
    return ToHex(reinterpret_cast<uint64_t>(address), sizeof(uint64_t), bNotation);
}

auto
ToHex(uint64_t decimal, size_t width, bool bNotation) -> std::string {
...
auto
ToDecimal(const std::string& hexStr) -> uint64_t {
    uint64_t decimal = 0;
...
auto
ToDecimal(uint64_t hex, size_t width) -> std::string {
    std::ostringstream stream;
...
auto
ColorToHex(float colorArray[3], bool bNotation) -> std::string {
...
auto
HexToDecimal(const std::string& hexStr) -> uint64_t {
    uint64_t decimal = 0;
    std::stringstream stream;
    stream << std::right << std::uppercase << std::hex << RemoveAllChars(hexStr, '#');
    stream >> decimal;
...
auto
HexToIntPointer(const std::string& hexStr) -> uintptr_t {
    uintptr_t decimal = 0;
    std::stringstream stream;
...
namespace Files {
void
FindPngImages(const fs::path& directory, std::unordered_map<std::string, fs::path>& imageMap) {
...
void
OpenFolder(const fs::path& folderPath) {
...
auto
get_text_content(const fs::path& file_path) -> std::string {
...
auto
get_json(const fs::path& file_path) -> json {
    json j;
    ...
    try {
        ...
        file >> j;
    } catch (const std::exception& e) { LOG("[ERROR] Unable to read '{}': {}", file_path.filename().string(), e.what()); }

    return j;
}

auto
write_json(const fs::path& file_path, const json& j) -> bool {
    try {
...
void
FilterLinesInFile(const fs::path& filePath, const std::string& startString) {
...
auto
GetCommandOutput(const char* cmd) -> std::string {
    ...
    return Format::TrimBeginningAndEndWhitespace(
        result); // trim whitespace characters from beginning/end (it'll make filepaths invalid)
}

// Function to trim whitespace and invisible characters (such as newlines)
auto
CleanPathStr(const std::string& path) -> std::string {
    std::string cleanedPath = path;

    // Remove trailing whitespace or invisible characters like '\n', '\r'
    cleanedPath.erase(
        std::remove_if(cleanedPath.begin(), cleanedPath.end(), [](unsigned char c) { return std::isspace(c); }), cleanedPath.end());
...
void
terminate_created_process(ProcessHandles& pi) {
...
auto
create_process_from_command(const std::string& command) -> CreateProcessResult {
    ...
    if (CreateProcessW(nullptr,  // Application name (set NULL to use command)
