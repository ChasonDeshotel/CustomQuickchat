#include "../../lib/imgui_bm/imgui.h"
#include "../../lib/imgui_bm/imgui_searchablecombo.h"
#include "../include/main.h"
#include "LobbyInfo.h"

void
CustomQuickchat::RenderSettings() {
    ...
    if (ImGui::BeginChild("PluginSettingsSection", ImVec2(0, content_height))) {
        ...
        if (enabled) {
            ...
            if (ImGui::Button("Open Bindings Menu")) {
                GAME_THREAD_EXECUTE(cvarManager->executeCommand("togglemenu " + GetMenuName()););
            }
        }
    }
    ImGui::EndChild();
...
void
CustomQuickchat::GeneralSettings() {
...
void
CustomQuickchat::ChatTimeoutSettings() {
...
void
CustomQuickchat::SpeechToTextSettings() {
    ...
    if (!ws_is_connected_to_server && !connecting_to_ws_server.load()) {
        if (ImGui::Button("Start##websocket")) {
            ...
            GAME_THREAD_EXECUTE_CAPTURE(start_ws_connection();, start_ws_connection);
        }
    } else {
        if (ImGui::Button("Stop##websocket")) {
            ...
            GAME_THREAD_EXECUTE_CAPTURE(stop_ws_connection();, stop_ws_connection);
        }
    }
    ...
    if (ImGui::IsItemHovered()) {
        ImGui::SetTooltip(
            "Uses a stored calibration value, eliminating the need to calibrate mic before every attempt (can be faster)");
    }
    ...
    if (speechToTextNotificationsOn) {
        ...
        if (ImGui::Button("Test")) {
            GAME_THREAD_EXECUTE_CAPTURE(Instances.SpawnNotification("Terry A Davis",
...
void
CustomQuickchat::LastChatSettings() {
    ...
    if (ImGui::Button("Clear##chatLog")) {
        GAME_THREAD_EXECUTE(LobbyInfo.clear_stored_chats(););
    }
...
void
CustomQuickchat::RenderAllBindings() {
...
void
CustomQuickchat::RenderBindingDetails() {
...
void
CustomQuickchat::RenderChatDetails(Binding& selectedBinding) {
...
void
CustomQuickchat::RenderBindingTriggerDetails(Binding& selectedBinding) {
    ...
    if (ImGui::Button("Save")) {
        ...
        GAME_THREAD_EXECUTE(determine_quickchat_labels();

            auto chat = Instances.GetInstanceOf<UGFxData_Chat_TA>();
            if (chat) { apply_all_custom_qc_labels_to_ui(chat); }
...
void
CustomQuickchat::RenderAllVariationListNames() {
...
void
CustomQuickchat::RenderVariationListDetails() {
