...
uintptr_t
InstancesComponent::FindPattern(HMODULE module, const unsigned char* pattern, const char* mask) {
    MODULEINFO info = {};
...
uintptr_t
InstancesComponent::GetGNamesAddress() {
...
uintptr_t
InstancesComponent::GetGObjectsAddress() {
...
bool
InstancesComponent::AreGObjectsValid() {
...
bool
InstancesComponent::AreGNamesValid() {
...
bool
InstancesComponent::CheckGlobals() {
...
class UClass*
InstancesComponent::FindStaticClass(const std::string& className) {
...
class UFunction*
InstancesComponent::FindStaticFunction(const std::string& className) {
...
class UEngine*
InstancesComponent::IUEngine() {
    return UEngine::GetEngine();
}

class UAudioDevice*
InstancesComponent::IUAudioDevice() {
    return UEngine::GetAudioDevice();
}

class AWorldInfo*
InstancesComponent::IAWorldInfo() {
    return UEngine::GetCurrentWorldInfo();
}

class UCanvas*
InstancesComponent::IUCanvas() {
    return I_UCanvas;
}

class AHUD*
InstancesComponent::IAHUD() {
    return I_AHUD;
}

class UFileSystem*
InstancesComponent::IUFileSystem() {
    return (UFileSystem*)UFileSystem::StaticClass();
}

class UGameViewportClient*
InstancesComponent::IUGameViewportClient() {
    return I_UGameViewportClient;
}

class ULocalPlayer*
InstancesComponent::IULocalPlayer() {
...
InstancesComponent::IAPlayerController() {
    return I_APlayerController;
}

struct FUniqueNetId
InstancesComponent::GetUniqueID() {
...
UGFxDataStore_X*
InstancesComponent::GetDataStore() {
    if (dataStore) {
        return dataStore;
    }
...
UOnlinePlayer_X*
InstancesComponent::GetOnlinePlayer() {
    if (onlinePlayer) {
        return onlinePlayer;
    }
...
FString
InstancesComponent::NewFString(const std::string& str) {
    // wchar_t* tempWstr = Format::ToWcharString(str);		// uses new wchar_t*

    // FString gameCreatedFStr = UObject::Trim(tempWstr);	// have the game create an FString (using UE), rather than using new
    // wchar_t* directly (which causes crashes)
    ...
    return UObject::Trim(StringUtils::ToWideString(str)
            .data()); // have the game create an FString (using UE), rather than using new wchar_t* directly (which causes crashes)
}

FName
InstancesComponent::FindFName(const std::string& str) {
