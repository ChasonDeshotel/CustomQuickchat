...
SpeechToText::start_websocket_stuff(bool onLoad) {
    ...
    auto ws_client_setup = [this, onLoad]() {
        ...
        DELAY_CAPTURE(START_WS_CLIENT_DELAY, start_the_client();, start_the_client);
    };
    ...
    if (onLoad) {
        // if called from onload, wait 1 second before starting websocket stuff (so stored websocket_port cvar value gets a chance to
        // load)
        DELAY_CAPTURE(1.0f, start_server_then_client();, start_server_then_client);
    } else {
...
SpeechToText::start_websocket_server() {
    ...
    // start websocket sever (spawn python process)
    std::string command =
        CreateCommandString(speechToTextExePath.string(), { std::to_string(websocket_port) }); // args: py exe, websocket port
    ...
    if (response_data.contains("success")) {
        if (response_data["success"]) {
            if (response_data.contains("transcription")) {
                ...
                // send chat
                GAME_THREAD_EXECUTE_CAPTURE(SendChat(text, binding.chatMode);, text, binding);
            } else {
