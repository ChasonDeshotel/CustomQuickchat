...
WebsocketClientManager::WebsocketClientManager(std::function<void(json serverResponse)> response_callback,
    std::atomic<bool>& connecting_to_ws_server)
...
auto
WebsocketClientManager::StartClient(int port) -> bool {
...
auto
WebsocketClientManager::StopClient() -> bool {
    std::lock_guard<std::mutex> lock(connection_mutex);
    ...
    try {
        ws_client.stop();
    } catch (const std::exception& e) { LOG("[WebsocketManager] Error stopping client: {}", e.what()); }
...
void
WebsocketClientManager::SendEvent(const std::string& eventName, const json& dataJson) {
    ...
    } catch (const std::exception& e) { LOG("[WebsocketManager] Exception while sending message: {}", e.what()); }
}

void
WebsocketClientManager::OnWsOpen(connection_hdl hdl) {
...
void
WebsocketClientManager::OnWsClose(connection_hdl hdl) {
...
void
WebsocketClientManager::OnWsMessage(connection_hdl hdl, PluginClient::message_ptr msg) {
    is_connected.store(true);
    connecting_to_server.store(false);
    ...
    } catch (const std::exception& e) { LOG("[WebsocketManager] Error processing message: {}", e.what()); }
}

void
WebsocketClientManager::set_connected_status(bool connected) {
    is_connected = connected;
}

auto
WebsocketClientManager::get_port_str() const -> std::string {
    return port_num_str;
}
